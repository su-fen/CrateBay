...
impl LinuxHypervisor {
    ...
    fn create_kvm_vm(
        ...
        let kernel_path_str: Option<String> = config
            .kernel_path
            .clone()
            .or_else(|| std::env::var("CRATEBAY_KVM_KERNEL").ok());
        ...
        if let Some(ref kp) = kernel_path_str {
            ...
            let initrd_path_str: Option<String> = config
                .initrd_path
                .clone()
                .or_else(|| std::env::var("CRATEBAY_KVM_INITRD").ok());
            ...
            // Set up command line.
            let cmdline = std::env::var("CRATEBAY_KVM_CMDLINE")
    ...
    fn spawn_virtiofsd(
        ...
        let socket_path = format!("/tmp/cratebay-virtiofs-{}.sock", tag);
...
mod tests {
    ...
    /// (`CRATEBAY_CONFIG_DIR`, `CRATEBAY_DATA_DIR`).  Without this, parallel
    ...
    fn temp_config_dir() -> (
        ...
        let g1 = EnvGuard::set("CRATEBAY_CONFIG_DIR", &path);
        let g2 = EnvGuard::set("CRATEBAY_DATA_DIR", &path);
        (path, g1, g2, lock)
    }
    ...
    fn vm_dir_path_contains_id() {
        let _g = EnvGuard::set("CRATEBAY_DATA_DIR", "/tmp/cb-linux-test");
